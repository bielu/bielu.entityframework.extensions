...
public class VersionedDbContextTests
{
    ...
    public async Task Save_first_version_is_classified_as_Initial(TestProvider provider)
    {
        ...
        var result = await harness.Context.SaveAsync<Content, Guid, Guid>(
            entityId, T1, new Content { Title = "v1", Body = "body 1" });
    ...
    public async Task Save_second_version_with_later_EffectiveAt_is_Current(TestProvider provider)
    {
        ...
        var second = await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T2, new Content { Title = "v2" });
    ...
    public async Task Save_back_dated_version_is_classified_as_Archive(TestProvider provider)
    {
        ...
        await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" });
        await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T3, new Content { Title = "v3" });

        // Insert v2 in between the two existing versions.
        var middle = await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T2, new Content { Title = "v2" });
    ...
    public async Task GetCurrentAsync_after_inserting_in_between_returns_latest_by_EffectiveAt(TestProvider provider)
    {
        ...
        var current = await harness.Context.GetCurrentAsync<Content, Guid, Guid>(entityId);
    ...
    public async Task GetCurrentAsync_with_asOf_returns_version_active_at_that_time(TestProvider provider)
    {
        ...
        (await harness.Context.GetCurrentAsync<Content, Guid, Guid>(entityId, T1_5))!.Title.ShouldBe("v1");
        (await harness.Context.GetCurrentAsync<Content, Guid, Guid>(entityId, T2_5))!.Title.ShouldBe("v2");
        (await harness.Context.GetCurrentAsync<Content, Guid, Guid>(entityId, T3.AddDays(1)))!.Title.ShouldBe("v3");
    }
    ...
    public async Task GetAllVersionsAsync_returns_chronological_order(TestProvider provider)
    {
        ...
        var all = await harness.Context.GetAllVersionsAsync<Content, Guid, Guid>(entityId);
    ...
    public async Task GetVersionCountAsync_is_cheap_and_correct(TestProvider provider)
    {
        ...
        (await harness.Context.GetVersionCountAsync<Content, Guid, Guid>(entityId)).ShouldBe(0);
    ...
    public async Task GetNeighborsAsync_returns_predecessor_and_successor(TestProvider provider)
    {
        ...
        var n = await harness.Context.GetNeighborsAsync<Content, Guid, Guid>(entityId, T2);
    ...
    public async Task UpdateAsync_throws_when_no_existing_versions(TestProvider provider)
    {
        ...
        await Should.ThrowAsync<InvalidOperationException>(async () =>
            await harness.Context.UpdateAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" }));
    }
    ...
    public async Task UpdateAsync_works_when_aggregate_already_exists(TestProvider provider)
    {
        ...
        await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" });
        var update = await harness.Context.UpdateAsync<Content, Guid, Guid>(entityId, T2, new Content { Title = "v2" });
    ...
    public async Task EffectiveAt_collision_is_detected(TestProvider provider)
    {
        ...
        await Should.ThrowAsync<EffectiveAtCollisionException>(async () =>
            await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1-collision" }));
    }
    ...
    public async Task SoftDelete_marks_subsequent_GetCurrent_as_null(TestProvider provider)
    {
        ...
        await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" });
        await harness.Context.SoftDeleteVersionAsync<Content, Guid, Guid>(entityId, T2, new Content { Title = "tombstone" });

        var current = await harness.Context.GetCurrentAsync<Content, Guid, Guid>(entityId);
        current.ShouldBeNull();

        // Historical reads still work (the tombstone itself is a version).
        var history = await harness.Context.GetAllVersionsAsync<Content, Guid, Guid>(entityId);
    ...
    public async Task RecordedAt_uses_clock(TestProvider provider)
    {
        ...
        var first = await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" });
    ...
    public async Task InPlace_modification_throws_by_default(TestProvider provider)
    {
        ...
        var first = (await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T1, new Content { Title = "v1" })).Entity;
    ...
    public async Task Clock_abstraction_is_consulted_per_call()
    {
        ...
        var second = (await harness.Context.SaveAsync<Content, Guid, Guid>(entityId, T2, new Content { Title = "v2" })).Entity;
